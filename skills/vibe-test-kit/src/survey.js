...
.sv-btn{padding:12px 36px;border-radius:12px;border:none;background:linear-gradient(135deg,#475569,#1e293b);color:#fff;font-size:.9rem;font-weight:700;cursor:pointer;font-family:inherit;box-shadow:0 4px 20px rgba(15,23,42,.25);transition:all .2s}\
.sv-btn:hover{filter:brightness(1.05);transform:translateY(-1px)}\
.sv-tp{position:fixed;bottom:24px;right:24px;z-index:9990;width:300px;background:#fff;border:1px solid #e2e8f0;border-radius:16px;box-shadow:0 12px 48px rgba(15,23,42,.12);overflow:hidden;animation:svU .4s;transition:all .3s ease}\
.sv-tp.min{width:auto;max-width:280px;border-radius:28px;cursor:pointer;background:linear-gradient(135deg,#1e293b,#334155);border:none;box-shadow:0 4px 24px rgba(15,23,42,.25),0 0 0 0 rgba(15,23,42,.08);position:fixed;bottom:24px;right:24px;top:auto;animation:svBounce .5s ease,svGlow 2s ease-in-out infinite;overflow:visible}\
.sv-tp.min.tucked{transform:none;animation:none;box-shadow:none;background:transparent;border:none;padding:0;width:30px;height:60px;right:0;cursor:default;overflow:visible}\
.sv-tp.min.tucked>*:not(.sv-tuck-handle){display:none!important}\
.sv-tp.min.tucked .sv-tuck-handle{display:flex}\
.sv-tuck-handle{display:none;position:absolute;right:0;top:0;width:30px;height:60px;background:linear-gradient(135deg,#1e293b,#334155);border-radius:14px 0 0 14px;align-items:center;justify-content:center;cursor:pointer;color:#fff;font-size:.78rem;font-weight:700;letter-spacing:.5px;box-shadow:-4px 4px 16px rgba(15,23,42,.18);transition:all .25s ease}\
.sv-tuck-handle:hover{width:36px;background:linear-gradient(135deg,#0f172a,#1e293b)}\
.sv-tuck-handle .sv-tuck-num{font-size:.62rem;font-weight:800;color:#fff;line-height:1.2;text-align:center}\
.sv-tuck-handle .sv-tuck-arrow{font-size:.55rem;color:rgba(255,255,255,.7);margin-top:3px}\
.sv-tp-tuck-btn{display:none;position:absolute;left:8px;top:50%;transform:translateY(-50%);width:18px;height:22px;border:none;background:rgba(255,255,255,.16);color:#fff;border-radius:5px;cursor:pointer;font-size:.6rem;line-height:1;align-items:center;justify-content:center;font-family:inherit;transition:all .2s;z-index:2;padding:0}\
.sv-tp.min:not(.tucked) .sv-tp-tuck-btn{display:flex}\
.sv-tp-tuck-btn:hover{background:rgba(255,255,255,.28)}\
.sv-tp.min:not(.tucked) .sv-tp-h{padding-left:34px}\
...
.sv-tp.min:hover{transform:translateY(-3px);box-shadow:0 6px 32px rgba(15,23,42,.32)}\
.sv-tp.min.tucked:hover{transform:none;box-shadow:none}\
...
@keyframes svGlow{0%,100%{box-shadow:0 4px 24px rgba(15,23,42,.3),0 0 0 0 rgba(15,23,42,.1)}50%{box-shadow:0 4px 24px rgba(15,23,42,.3),0 0 0 8px rgba(15,23,42,.06)}}\
.sv-fab{display:none;align-items:center;gap:8px;font-size:.72rem;color:#94a3b8;white-space:nowrap}\
...
.sv-fab-ring svg circle{fill:none;stroke:#1e293b;stroke-width:2.5;stroke-linecap:round;transition:stroke-dashoffset .5s ease}\
.sv-fab-num{font-size:.65rem;font-weight:800;color:#1e293b}\
...
.sv-tp-p{font-size:.65rem;color:#1e293b;font-weight:600}\
...
.sv-ci:focus{border-color:#475569}\
.sv-mo-sub{margin-top:12px;width:100%;padding:10px;border-radius:10px;border:none;background:linear-gradient(135deg,#475569,#1e293b);color:#fff;font-size:.8rem;font-weight:600;cursor:pointer;font-family:inherit;opacity:.4;pointer-events:none;transition:all .2s}\
...
.sv-sam-s::-webkit-slider-thumb{-webkit-appearance:none;width:22px;height:22px;border-radius:50%;background:#1e293b;cursor:pointer;box-shadow:0 2px 8px rgba(15,23,42,.3)}\
.sv-sam-v{font-size:.7rem;color:#1e293b;font-weight:700;min-width:20px;text-align:center}\
.sv-exit{position:fixed;bottom:24px;right:24px;z-index:10002;width:380px;max-height:calc(100vh - 48px);background:#fff;border:1px solid #e2e8f0;border-radius:18px;overflow:hidden;display:flex;flex-direction:column;box-shadow:0 16px 60px rgba(0,0,0,.15);animation:svU .4s}\
.sv-exit-c{display:flex;flex-direction:column;flex:1;min-height:0;overflow:hidden}\
.sv-exit-h{padding:18px 22px 12px;border-bottom:1px solid #f1f5f9;background:#fff;flex-shrink:0}\
.sv-exit-h h2{font-size:1rem;font-weight:800;color:#1e293b;margin:0 0 4px}\
.sv-exit-h p{font-size:.7rem;color:#94a3b8}\
.sv-exit-b{padding:16px 22px 22px;overflow-y:auto;flex:1}\
...
.sv-sec-tag{font-size:.58rem;color:#1e293b;background:rgba(15,23,42,.08);padding:2px 7px;border-radius:5px;font-weight:600}\
...
.sv-sq-o:hover{background:rgba(15,23,42,.06);border-color:rgba(15,23,42,.2)}\
.sv-sq-o.sel{background:rgba(15,23,42,.1);border-color:#475569;color:#0f172a}\
...
.sv-oi:focus{border-color:#475569}\
...
.sv-wr input:focus{border-color:#475569}\
...
.sv-thx{text-align:center;padding:28px 22px}\
.sv-thx-ic{font-size:2.2rem;margin-bottom:10px}\
.sv-thx-t{font-size:1.05rem;font-weight:800;color:#1e293b;margin-bottom:6px}\
.sv-thx-s{font-size:.72rem;color:#64748b;line-height:1.55;margin-bottom:18px}\
.sv-thx-m{display:flex;gap:14px;padding:12px 16px;background:#f8fafc;border-radius:12px;border:1px solid #e2e8f0;justify-content:center}\
...
@keyframes svP{0%,100%{box-shadow:0 0 0 0 rgba(30,41,59,0)}50%{box-shadow:0 0 0 4px rgba(30,41,59,.15)}}\
@media(max-width:640px){.sv-tp{width:240px;right:8px;bottom:8px}.sv-mo{width:300px;right:12px;bottom:12px}.sv-exit{width:94%;right:3%;bottom:12px}}\
';
...
c+='</div>';
c+='<div style="text-align:left;margin:0 auto 16px;max-width:340px;padding:12px 16px;border-radius:12px;background:rgba(30,41,59,.06);border:1px solid rgba(30,41,59,.12)">';
c+='<div style="font-size:.68rem;color:#475569;font-weight:600;margin-bottom:6px">🎯 你的任务</div>';
...
document.getElementById('svStart').onclick=function(){ov.style.animation='svF .3s reverse forwards';setTimeout(function(){ov.remove();if(cfgTasks)showTaskPanel();saveState();rec('survey_welcome_done',{config:configKey()})},300)};
...
document.body.appendChild(tpEl);renderTaskPanel();
// 8秒后自动收起为气泡
...
var h='<div class="sv-tp-h"><button class="sv-tp-tuck-btn" id="svTpTuck" title="贴边收起">»</button><div class="sv-tp-t"><span>📋</span> <span class="sv-tp-p">'+done+'/'+tasks.length+'</span>';
// 气泡模式内容
...
// 贴边时的拉手提示（点击展开）
h+='<div class="sv-tuck-handle" id="svTuckHandle"><div class="sv-tuck-num">'+done+'/'+tasks.length+'</div><div class="sv-tuck-arrow">◀</div></div>';
...
// 当前任务 — 大号显示
h+='<div style="padding:10px 12px;border-radius:10px;background:rgba(30,41,59,.06);border:1px solid rgba(30,41,59,.15);margin-bottom:8px">';
h+='<div style="font-size:.62rem;color:#475569;font-weight:600;margin-bottom:4px">当前任务 · 第 '+(curIdx+1)+' 步</div>';
...
// 贴边收起按钮（仅气泡模式可见）
var tuckBtn=document.getElementById('svTpTuck');
if(tuckBtn){tuckBtn.onclick=function(e){e.stopPropagation();tpEl.classList.toggle('tucked');try{sessionStorage.setItem('sv_tucked_'+uid,tpEl.classList.contains('tucked')?'1':'0');}catch(_){}}}
tpEl.onclick=function(e){
  if(tpEl.classList.contains('tucked')){tpEl.classList.remove('tucked');try{sessionStorage.setItem('sv_tucked_'+uid,'0');}catch(_){}return;}
  if(tpEl.classList.contains('min'))tpEl.classList.remove('min')
};
// 恢复贴边状态
try{if(sessionStorage.getItem('sv_tucked_'+uid)==='1'&&tpEl.classList.contains('min'))tpEl.classList.add('tucked');}catch(_){}
}
// 每秒更新计时器
...
// 显示感谢
ov.querySelector('.sv-exit-c').innerHTML='<div class="sv-thx"><div class="sv-thx-ic">🎉</div><div class="sv-thx-t">感谢你的参与！</div><div class="sv-thx-s">你的反馈对我们改进产品非常重要。<br>问卷数据已保存，你现在可以关闭页面了。</div><div class="sv-thx-m"><div class="sv-thx-v"><div class="vn" style="color:#475569">'+susScore.toFixed(0)+'</div><div class="vl">SUS 分数</div></div><div class="sv-thx-v"><div class="vn" style="color:'+(nps>=9?'#10b981':nps>=7?'#f59e0b':'#ef4444')+'">'+nps+'</div><div class="vl">NPS 评分</div></div><div class="sv-thx-v"><div class="vn" style="color:#f59e0b">'+Math.round((Date.now()-startTs)/1000/60)+'<span style="font-size:.7rem">min</span></div><div class="vl">测试用时</div></div></div></div>';
}
...
// 有恢复的进度？直接显示气泡（跳过欢迎页）
var hasSaved=!!saved;  // 有任何 saved 状态说明已经过了欢迎页
...
// 没有欢迎页，直接显示任务面板
if(cfgTasks){showTaskPanel();saveState();}
}
}
